...
const isPhone = defineModel('isPhone')
const isPL0 = defineModel('isPL0')

const onPhone = (phone) => {
  isPL0.value = phone.length === 0;
...
<template>
  ...
  :class="{
    'input__false': !isPL0 && !isPhone,
    'input__true': !isPL0 && isPhone,
  }">
    <Input
        placeholder="Телефон"
        @onChange="onPhone"
    />
    <div v-if="!isPL0 && isPhone" class="phone-input__mark">
